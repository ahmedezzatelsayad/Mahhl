...
export const SITE_NAME = 'متاجر جارفكس';
...
export const DEFAULT_SEO: SeoSettings = {
  siteTitle: 'متاجر جارفكس | منصة دروب شيبنج رقم 1 في الكويت',
  titleTemplate: '%s | متاجر جارفكس',
  description:
    'متاجر جارفكس — أول منصة دروب شيبنج في الكويت. سوّق أكثر من 2600 منتج وكل منتج عليه عمولة مقترحة من 1 إلى 10 د.ك تختارها بمزاجك — بدون رأس مال وبدون هم الشحن، وطلبات عملائك توصلهم بتوصيل سريع لكل المحافظات مع الدفع عند الاستلام.',
  keywords:
    'دروب شيبنج الكويت, منصة دروب شيبنج, ربح من الانترنت الكويت, تسويق بالعمولة الكويت, عمولات المسوقين, dropshipping Kuwait, متاجر جارفكس, تسوق اونلاين الكويت, شراء اونلاين, دفع عند الاستلام الكويت, توصيل الكويت, عروض الكويت, خصومات الكويت',
  siteUrl: '',
  googleVerification: '',
  bingVerification: '',
};
...
export function withBrand(title: string): string {
  const t = title.trim();
  if (!t || t.includes('متاجر جارفكس')) return t;
  const branded = `${t} | متاجر جارفكس`;
...
export function productKeywords(
  ...
  const fallback = [p.name, p.category?.name || '', 'شراء أونلاين الكويت', 'متاجر جارفكس'];
...
export function productDescription(p: ProductLike): string {
  ...
  const core =
    clean.length >= 50
      ...
      : `اشترِ ${p.name} من متاجر جارفكس بسعر ${formatKwd(p.salePrice)} د.ك`;
...
export function organizationJsonLd(url: string) {
  return {
    ...
    slogan: 'منصة دروب شيبنج رقم 1 في الكويت — سوّق واربح عمولة على كل طلب',
    areaServed: {
      '@type': 'Country',
      name: 'الكويت (Kuwait)',
    },
  ...
  const faqs: { q: string; a: string }[] = [
    {
      ...
      a: `نعم، متاجر جارفكس يوصّل لجميع محافظات الكويت (العاصمة، حولي، الفروانية، الأحمدي، الجهراء، مبارك الكبير). سعر التوصيل ${formatKwd(shipping.price)} د.ك.${freeText}`,
    },
    {
      q: 'ما هي طرق الدفع المتاحة في متاجر جارفكس؟',
      a: 'الدفع عند الاستلام (COD) — تدفع نقداً للمندوب عند استلام طلبك. جميع الأسعار بالدينار الكويتي.',
    },
    {
      q: 'كم عدد المنتجات في متاجر جارفكس؟',
      a: 'أكثر من 2600 منتج في 38 فئة: أجهزة كهربائية، مستلزمات مطبخ، أحزمة ومشدات، ألعاب، أدوات، عناية شخصية والمزيد.',
    },
    {
      q: 'كيف أطلب من متاجر جارفكس؟',
      a: 'اختر المنتج، أضفه للسلة، اكتب اسمك ورقم هاتفك ومحافظتك ومنطقتك، ثم أكّد الطلب — سنتصل بك لتأكيد التوصيل.',
    },
    {
      q: 'شلون أربح من متاجر جارفكس كم مسوّق؟',
      a: 'سجّل مجاناً كبوابة المسوقين (سوّق معنا)، بياصلك كود ورابط خاص فيك — شارك أي منتج من أكثر من 2600 منتج، وكل طلب يوصَل عبر رابطك يتحسبلك عمولة تُضاف فوق سعر المنتج — المقترحة على كل منتج من 1 إلى 10 د.ك وإنت حر تختار عمولتك بمزاجك — بدون رأس مال وبدون هم الشحن أو التحصيل.',
    },
    {
      q: 'متى وكيف أستلم عمولتي كم مسوّق؟',
      a: 'عمولتك تتحسب تلقائياً في محفظتك عند تسليم كل طلب، وتطلب سحبها من لوحة المسوق متى ما تبى — والمحاسبة كاملة وشفافة: تقدر تشوف كل الطلبات وحالتها وعمولاتها لحظة بلحظة.',
    },
  ];
  return {
    '@context': 'https://schema.org',
...
export function faqEntries(): { q: string; aKey: 'shipping' | 'cod' | 'count' | 'how' }[] {
  return [
    ...
    { q: 'كم عدد المنتجات في متاجر جارفكس؟', aKey: 'count' },
    { q: 'كيف أطلب من متاجر جارفكس؟', aKey: 'how' },
  ];
}
...
export async function resolveSeoPage(sp: Record<string, string | string[] | undefined>): Promise<SeoPageData> {
  ...
  const INFO_META: Record<InfoPageKind, { title: string; description: string }> = {
    about: {
      title: 'من نحن — متاجر جارفكس منصة دروب شيبنج في الكويت',
      description: 'متاجر جارفكس: أول منصة دروب شيبنج في الكويت — أكثر من 2,600 منتج بعمولات مقترحة من 1 إلى 10 د.ك للمسوقين — سجّل مجاناً وافتح الكتالوج الكامل واختر عمولتك بمزاجك.',
    },
    contact: {
      title: 'تواصل معنا — واتساب متاجر جارفكس',
      description: 'تواصل مع فريق متاجر جارفكس على واتسوب يومياً من 9 صباحاً حتى 11 مساءً — نرد عليك بأسرع وقت.',
    },
    faq: {
      title: 'الأسئلة الشائعة — تسوق واربح مع منصة متاجر جارفكس',
      description: 'شلون أطلب؟ شلون أدفع؟ شلون أربح كم مسوّق؟ — إجابات واضحة على أكثر أسئلة عملائنا والمسوقين في الكويت.',
    },
    shipping: {
      title: 'الشحن والتوصيل — لكل محافظات الكويت | متاجر جارفكس',
    ...
    returns: {
      title: 'الاستبدال والاسترجاع — خلال 3 أيام | متاجر جارفكس',
      description: 'سياسة استبدال واسترجاع واضحة: خلال 3 أيام من الاستلام للمنتجات بحالتها الأصلية — تواصل معنا على الواتساب.',
    },
    privacy: {
      title: 'سياسة الخصوصية — بياناتك أمانة | متاجر جارفكس',
      description: 'نجمع فقط الاسم والهاتف والعنوان لتنفيذ طلبك — لا نخزن بطاقات ولا نشارك بياناتك مع أي طرف ثالث.',
    },
    terms: {
      title: 'الشروط والأحكام | منصة متاجر جارفكس للدروب شيبنج',
      description: 'شروط استخدام منصة متاجر جارفكس: الأسعار بالدينار الكويتي، الدفع عند الاستلام، التوصيل لكل محافظات الكويت، وعمولات مسوّقين مقترحة من 1 إلى 10 د.ك تُضاف فوق سعر كل منتج.',
    },
    'affiliate-program': {
      title: 'برنامج المسوقين — سوّق واربح عمولة من 1 إلى 10 د.ك على كل منتج | متاجر جارفكس',
      description: 'سجّل مجاناً في برنامج التسويق بالعمولة في متاجر جارفكس: أكثر من 2,600 منتج جاهز، عمولة مقترحة من 1 إلى 10 د.ك على كل منتج — وإنت تختار، رابط إحالة خاص فيك, محفظة شفافة وسحب وقت ما تبى — بدون رأس مال وبدون هم الشحن.',
    },
    'guide-ads': {
      title: 'أفضل ممارسات الدعاية والإعلانات في الكويت — مركز المسوقين | متاجر جارفكس',
      description: 'دليل شامل للمسوقين في الكويت: قنوات الإعلان الأقوى (سناب شات، تيك توك، إنستقرام)، قواعد الإعلان اللي يبيع، الميزانيات الذكية، والأرقام الواقعية المتوقعة — من منصة متاجر جارفكس للدروب شيبنج.',
    },
    'guide-campaigns': {
      title: 'دليل الحملات والمواسم الكويتية للمسوقين | متاجر جارفكس',
      description: 'تقويم مواسم الكويت (رمضان، الأعياد، العودة للمدارس، الهلا نوفمبر، الجمعة البيضاء) + قوالب حملات جاهزة خطوة بخطوة + نصوص إعلانية بالعامية الكويتية جاهزة للنسخ.',
    },
  };
