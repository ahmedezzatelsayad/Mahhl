...
export async function buildFaqJsonLd(url: string) {
  ...
  const faqs: { q: string; a: string }[] = [
    ...
    },
    {
      ...
      a: 'سجّل مجاناً كبوابة المسوقين (سوّق معنا)، بياصلك كود ورابط خاص فيك — شارك أي منتج من أكثر من 2600 منتج، وكل طلب يوصَل عبر رابطك يتحسبلك عمولة من 1 إلى 2 د.ك حسب تنافسية المنتج، بدون رأس مال وبدون هم الشحن أو التحصيل.',
    },
    {
...
export async function resolveSeoPage(sp: Record<string, string | string[] | undefined>): Promise<SeoPageData> {
  ...
  const INFO_META: Record<InfoPageKind, { title: string; description: string }> = {
    about: {
      ...
      description: 'محل شوب: أول منصة دروب شيبنج في الكويت — أكثر من 2,600 منتج بعمولات 1–2 د.ك للمسوقين، وتسوق بالدينار الكويتي مع توصيل لكل المحافظات ودفع عند الاستلام.',
    },
    contact: {
    ...
    terms: {
      ...
      description: 'شروط استخدام منصة محل شوب: الأسعار بالدينار الكويتي، الدفع عند الاستلام، التوصيل لكل محافظات الكويت، وعمولات المسوقين 1–2 د.ك على كل طلب مسلّم.',
    },
    'affiliate-program': {
      title: 'برنامج المسوقين — سوّق واربح عمولة 1–2 د.ك على كل طلب | محل شوب',
      description: 'سجّل مجاناً في برنامج التسويق بالعمولة في محل شوب: أكثر من 2,600 منتج جاهز، عمولة 1–2 د.ك على كل طلب يوصَل، رابط إحالة خاص فيك، محفظة شفافة وسحب وقت ما تبى — بدون رأس مال وبدون هم الشحن.',
    },
    'guide-ads': {
