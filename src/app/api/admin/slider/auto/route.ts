...
export async function POST(req: NextRequest) {
  return requirePermission(req, 'slider', 'manage', async () => {
    ...
    const prompt = `أنت كاتب إعلانات أول لمتجر كويتي اسمه "متاجر جارفكس". اكتب نصوص ${picked.length} شرائح سلايدر للصفحة الرئيسية — كل شريحة تروّج منتجاً حقيقياً من القائمة، والنص سيظهر فوق صورة المنتج نفسها. الموقع ثنائي اللغة: اكتب لكل شريحة النسخة العربية والنسخة الإنجليزية (ترويجية طبيعية بأسلوب أمازون، ليست ترجمة حرفية).
...
{"slides":[{"index":1,"eyebrow":"","title":"","highlight":"","subtitle":"","ctaLabel":"","chips":["","",""],"eyebrowEn":"","titleEn":"","highlightEn":"","subtitleEn":"","ctaLabelEn":"","chipsEn":["","",""],"tone":"dark"}]}`;
    ...
    const slides = picked.map((p, i) => {
      ...
      const fallback: SlideDraft = {
        index: i + 1,
        eyebrow: p.isBestSeller ? 'الأكثر مبيعاً 🔥' : 'اختيار متاجر جارفكس',
        title: shortName,
        highlight: 'بسعر مميز',
