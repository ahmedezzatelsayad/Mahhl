...
export async function POST(req: NextRequest) {
  try {
    ...
    const system = `أنت "محل شوب" — مساعد تسوق ذكي لمتجر إلكتروني كويتي (الأسعار بالدينار الكويتي، دفع عند الاستلام، توصيل لكل محافظات الكويت).
...
لا تختلق منتجات أو أسعار غير موجودة في القائمة.`;
    ...
    if (!reply) {
      if (products.length) {
        reply =
          ...
          products.slice(0, 3).map((p) => `• ${p.name} — ${formatKwdPlain(p.price)} د.ك`).join('\n') +
        ...
        reply = 'هلا والله! شنو اللي تدور عليه اليوم؟ اكتب لي اسم المنتج وأجيبه لك بأحسن سعر 🛒';
      }
    }
    ...
    return NextResponse.json(
      { reply: 'صار خطأ بسيط بالاتصال.. جرّب مرة ثانية بعد شوي 🙏', products: [] },
      { status: 200 }
    );
  }
}
